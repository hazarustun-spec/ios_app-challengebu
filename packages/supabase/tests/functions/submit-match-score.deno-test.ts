...
async function setupMatch(suffix: string): Promise<{
  ...
  const alice = await createTestUser({
    email: `alice-sms-${suffix}@test.local`,
    genderCategory: 'erkek',
  });
  const bob = await createTestUser({
    email: `bob-sms-${suffix}@test.local`,
    genderCategory: 'erkek',
  });
...
Deno.test('submit-match-score: non-participant forbidden', async () => {
  ...
  try {
    ...
    assertEquals(
      ...
      true,
      `Expected 4xx rejection for non-participant, got ${r.status}`,
    );
  } finally {
