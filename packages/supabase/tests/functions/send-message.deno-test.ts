...
async function makeConversation(creatorId: string, targetId: string): Promise<string> {
...
Deno.test('send-message: delivers a message + creates a recipient notification', async () => {
  ...
  const a = await createTestUser({ email: `sm-a-${s}@test.local` });
  const b = await createTestUser({ email: `sm-b-${s}@test.local` });
  try {
...
Deno.test('send-message: a non-participant is rejected', async () => {
  const s = crypto.randomUUID().slice(0, 8);
  const a = await createTestUser({ email: `sm-x-${s}@test.local` });
  const b = await createTestUser({ email: `sm-y-${s}@test.local` });
  const c = await createTestUser({ email: `sm-z-${s}@test.local` });
  try {
    const convId = await makeConversation(a.userId, b.userId);
    const res = await invokeFunction(
