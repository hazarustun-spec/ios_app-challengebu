...
async function makeSeason(opts: { status: string; suffix: string }): Promise<string> {
  ...
  const { data: season, error: seasonErr } = await supa
    ...
    .insert({
      ...
      ends_at: '2026-12-31',
      finale_starts_at: '2026-12-20',
      finale_ends_at: '2026-12-30',
      status: opts.status,
    })
    .select('id')
    .single();
  if (!season || seasonErr)
    throw new Error(`makeSeason failed for suffix ${opts.suffix}: ${seasonErr?.message}`);
  return season.id;
}
...
Deno.test('close-season: applies soft ELO reset', async () => {
  ...
  const admin = await createTestUser({
    email: `admin-cs-${s}@test.local`,
    role: 'admin',
    genderCategory: 'erkek',
  });
  const alice = await createTestUser({
    email: `alice-cs-${s}@test.local`,
    genderCategory: 'erkek',
  });
  const supa = adminClient();
...
Deno.test('close-season: awards seasonal badges to standings + final winners', async () => {
  ...
  const { data: season, error: seasonErr } = await supa
    ...
    .insert({
      name: 'guz',
      year: csYear,
      starts_at: `${csYear}-09-01T00:00:00Z`,
      ends_at: `${csYear}-12-31T23:59:59Z`,
      finale_starts_at: `${csYear}-12-20T00:00:00Z`,
      finale_ends_at: `${csYear}-12-30T23:59:59Z`,
      status: 'finale',
    })
    .select('id')
    .single();
  if (!season || seasonErr)
    throw new Error(`season insert failed for suffix ${s}: ${seasonErr?.message}`);
  ...
  const { data: regMatch, error: regMatchErr } = await supa
    .from('matches')
    .insert({
      category: 'erkek_tek',
      format: 'bu_klasik',
      court_id: court.id,
      played_at: `${csYear}-10-15T18:00:00Z`,
      is_rated: true,
  ...
  const { data: tournament, error: tournErr } = await supa
    .from('tournaments')
    .insert({
      season_id: seasonId,
      category: 'erkek_tek',
  ...
  const { data: sf1Match, error: sf1Err } = await supa
    .from('matches')
    .insert({
      category: 'erkek_tek',
      format: 'bu_klasik',
      court_id: court.id,
      played_at: `${csYear}-12-21T18:00:00Z`,
      is_rated: true,
      team_a_player_ids: [players[0].userId],
      team_b_player_ids: [players[3].userId],
      score_team_a: 4,
      score_team_b: 1,
      winner_team: 'a',
      status: 'confirmed',
  ...
  const { error: sf1TmErr } = await supa.from('tournament_matches').insert({
    ...
    seed_a: 1,
    seed_b: 4,
    match_id: sf1Match.id,
  });
  if (sf1TmErr) throw new Error(`sf1 tournament_match insert failed: ${sf1TmErr.message}`);

  const { data: sf2Match, error: sf2Err } = await supa
    .from('matches')
    .insert({
      category: 'erkek_tek',
      format: 'bu_klasik',
      court_id: court.id,
      played_at: `${csYear}-12-22T18:00:00Z`,
      is_rated: true,
      team_a_player_ids: [players[1].userId],
      team_b_player_ids: [players[2].userId],
      score_team_a: 4,
      score_team_b: 2,
      winner_team: 'a',
      status: 'confirmed',
  ...
  const { error: sf2TmErr } = await supa.from('tournament_matches').insert({
    ...
    seed_a: 2,
    seed_b: 3,
    match_id: sf2Match.id,
  });
  if (sf2TmErr) throw new Error(`sf2 tournament_match insert failed: ${sf2TmErr.message}`);

  const { data: finalMatch, error: finalErr } = await supa
    .from('matches')
    .insert({
      category: 'erkek_tek',
      format: 'bu_klasik',
      court_id: court.id,
      played_at: `${csYear}-12-23T18:00:00Z`,
      is_rated: true,
      team_a_player_ids: [players[0].userId],
      team_b_player_ids: [players[1].userId],
      score_team_a: 4,
  ...
  try {
    ...
    if ((matchesCheck ?? []).length < 4) {
      throw new Error(
        `Expected 4 matches in season window, found ${matchesCheck?.length}: ${JSON.stringify(matchesCheck)}`,
      );
    }
