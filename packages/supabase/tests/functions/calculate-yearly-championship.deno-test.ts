...
Deno.test('calculate-yearly-championship: admin computes points across seasons', async () => {
  ...
  const alice = await createTestUser({
    email: `alice-cyc-${s}@test.local`,
    genderCategory: 'erkek',
  });
  const supa = adminClient();

  // Use a unique year far from real seasons to avoid collisions
  const year = 2090 + (Number.parseInt(s.slice(0, 2), 16) % 5);

  const { data: s1 } = await supa
    .from('seasons')
    .insert({
      name: 'guz',
      year,
      ...
      ends_at: `${year}-12-31`,
      finale_starts_at: `${year}-12-20`,
      finale_ends_at: `${year}-12-30`,
      status: 'closed',
    })
    .select('id')
    .single();
  const { data: s2 } = await supa
    ...
    .insert({
      ...
      ends_at: `${year}-06-30`,
      finale_starts_at: `${year}-06-20`,
      finale_ends_at: `${year}-06-30`,
      status: 'closed',
    })
    .select('id')
    .single();
  ...
  try {
    ...
    await teardownUsers([admin.userId, alice.userId], { seasonIds: [s1!.id, s2!.id] });
  }
});
