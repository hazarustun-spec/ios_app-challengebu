...
Deno.test('admin_cron_status: admin caller gets rows', async () => {
  ...
  try {
    const { data, error } = await jwtClient(admin.accessToken).rpc('admin_cron_status', {
      lim: 10,
    });
...
async function seedFinaleBracket(suffix: string): Promise<{
  ...
  const { data: tournament, error: tErr } = await supa
    .from('tournaments')
    .insert({
      season_id: season.id,
      category: 'erkek_tek',
...
Deno.test('admin_reorder_bracket_seeds: rewrites season_standings.rank by player order', async () => {
  ...
  try {
    ...
    const { error: rpcErr } = await jwtClient(admin.accessToken).rpc(
      'admin_reorder_bracket_seeds',
      {
    ...
    for (let i = 0; i < 8; i++) {
      assertEquals(
        ...
        reversed[i],
        `rank ${i + 1} should hold reversed[${i}]`,
      );
    }

    const { data: qf } = await supa
