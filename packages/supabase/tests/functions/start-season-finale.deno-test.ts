...
async function seedRatings(
  ...
  const admin = await createTestUser({
    email: `admin-ssf-${suffix}@test.local`,
    role: 'admin',
  ...
  for (let i = 0; i < count; i++) {
    const u = await createTestUser({
      email: `p${i}-ssf-${suffix}@test.local`,
      genderCategory: 'erkek',
    });
  ...
  if (!season || seasonErr)
    throw new Error(
      `seedRatings: season insert failed for suffix ${suffix}: ${seasonErr?.message}`,
    );
...
Deno.test('start-season-finale: 8 erkek_tek players → tournament + 4 first-round matches', async () => {
  ...
  try {
    ...
    if (result.tournamentCount < 1)
      throw new Error(`Expected at least 1 tournament, got ${result.tournamentCount}`);
...
Deno.test('start-season-finale: erkek_cift forms 4 teams from match history, seeds by avg rating', async () => {
  ...
  const ssfYear = 3000 + Number.parseInt(s.slice(0, 4), 16);
  const { data: season, error: seasonErr } = await supa
    .from('seasons')
    .insert({
      name: 'guz',
