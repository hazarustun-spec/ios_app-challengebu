...
Deno.test('advance-tournament-bracket: writes winner seed into parent slot', async () => {
  ...
  const { data: season, error: seasonErr1 } = await supa
    ...
    .insert({
      ...
      ends_at: '2027-01-25',
      finale_starts_at: '2027-01-16',
      finale_ends_at: '2027-01-25',
      status: 'finale',
    })
    .select('id')
    .single();
  if (!season || seasonErr1)
    throw new Error(`season insert failed for suffix ${s}: ${seasonErr1?.message}`);
  ...
  try {
    ...
    await teardownUsers([alice.userId, bob.userId], { seasonIds: [season!.id], matchIds: [m!.id] });
  }
});
...
Deno.test('advance-tournament-bracket: doubles (size 4) Final flips tournament to completed', async () => {
  ...
  const { data: season, error: seasonErr2 } = await supa
    ...
    .select('id')
    .single();
  if (!season || seasonErr2)
    throw new Error(`season insert failed for suffix ${s}: ${seasonErr2?.message}`);
