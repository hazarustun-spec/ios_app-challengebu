...
async function playedMatch(suffix: string): Promise<{
  ...
  const alice = await createTestUser({
    email: `alice-cm-${suffix}@test.local`,
    genderCategory: 'erkek',
  });
  const bob = await createTestUser({
    email: `bob-cm-${suffix}@test.local`,
    genderCategory: 'erkek',
  });
...
Deno.test('confirm-match: both confirm → ELO applied', async () => {
  ...
  try {
    ...
    if (aliceRating.rating <= 1200)
      throw new Error(`alice rating ${aliceRating.rating} should be > 1200`);
    if (bobRating.rating >= 1200)
