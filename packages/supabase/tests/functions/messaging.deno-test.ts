...
async function makeRequest(creatorId: string, targetId: string) {
...
Deno.test('messaging: participants can read/write under RLS, outsiders cannot', async () => {
  const a = await createTestUser({ email: 'msg-a@test.local' });
  const b = await createTestUser({ email: 'msg-b@test.local' });
  const c = await createTestUser({ email: 'msg-c@test.local' });
  ...
  // A (a participant + request creator) creates the conversation under RLS.
...
Deno.test('messaging: get_or_create_conversation is idempotent + mark_read works', async () => {
  const a = await createTestUser({ email: 'rpc-a@test.local' });
  const b = await createTestUser({ email: 'rpc-b@test.local' });
  const reqId = await makeRequest(a.userId, b.userId);
  const ca = userClient(a.accessToken);
  ...
  // A sends; B's unread count is 1, then mark_read drops it to 0.
...
Deno.test('messaging: a block prevents sending', async () => {
  const a = await createTestUser({ email: 'blk-a@test.local' });
  const b = await createTestUser({ email: 'blk-b@test.local' });
  const reqId = await makeRequest(a.userId, b.userId);
  const { low, high } = pair(a.userId, b.userId);
  const ca = userClient(a.accessToken);
  const cb = userClient(b.accessToken);
  ...
  // A can no longer send into the conversation.
