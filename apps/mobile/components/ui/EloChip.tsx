...
export function EloChip({ elo, delta }: EloChipProps) {
  ...
  return (
    ...
    >
      <Text className="font-num font-extrabold" style={{ fontSize: 16, color: colors.text }}>
        {elo}
      </Text>
      <View className="flex-row items-center" style={{ gap: 1 }}>
        <Icon name={up ? 'chevU' : 'chevD'} size={13} color={deltaColor} stroke={3} />
        <Text className="font-num font-bold" style={{ fontSize: 12.5, color: deltaColor }}>
          {Math.abs(delta)}
        </Text>
      </View>
    </View>
