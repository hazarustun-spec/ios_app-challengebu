// apps/mobile/app/leaderboard/index.tsx — Plan 8 Phase F (F13), live-wired.
//
// Main ladder view at `/leaderboard`. Wired to live Supabase data via
// useLadder(category), useCurrentSeason, useMyRankings, and useAuthStore.
//
// Sections (top → bottom):
...
export default function Leaderboard() {
  ...
  const validCatKeys = new Set<string>(CAT_CHIPS.map((c) => c.key));
  const myRankingMap: Partial<Record<Cat, number>> = {};
  for (const r of myRankingsQ.data ?? []) {
    if (validCatKeys.has(r.category)) {
