...
struct AwardPointIntent: LiveActivityIntent {
  ...
  func perform() async throws -> some IntentResult {
    ...
    guard let reqURL = URL(string: "\(url)/rest/v1/rpc/award_point") else { return .result() }

    // Build the award_point request for a given bearer token.
    func awardRequest(_ bearer: String) throws -> URLRequest {
    ...
    var (data, response) = try await URLSession.shared.data(for: try awardRequest(token))
    ...
    if (response as? HTTPURLResponse)?.statusCode == 401,
      ...
      (data, response) = try await URLSession.shared.data(for: try awardRequest(newToken))
    }
